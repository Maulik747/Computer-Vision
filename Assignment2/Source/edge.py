...
def conv(image, kernel):
    ...
    for i in range(pad_width0, Hi + pad_width0):
        for j in range(pad_width1, Wi + pad_width1):
            sum1 = padded[i-pad_width0:i+pad_width0+1,j-pad_width1:j+pad_width1+1]
            out[i-pad_width0,j-pad_width1] = (sum1 * h).sum()
    ### END YOUR CODE
...
def partial_x(img):
    ...
    ### YOUR CODE HERE
    pass
    x_filter=np.array([[0,0,0],[0.5, 0, -0.5],[0,0,0]])
...
def partial_y(img):
    ...
    ### YOUR CODE HERE
    pass
    y_filter=np.array([[0,-0.5,0],[0, 0, 0],[0,0.5,0]])
...
def gradient(img):
    ...
    theta=(np.rad2deg(np.arctan2(partial_x(img),partial_y(img)))+360)%360
...
def link_edges(strong_edges, weak_edges):
    ...
    while (len(queue)!=0):
        ...
        visited[x][y]=1
        neighbours=get_neighbors(x,y,H,W)
        for i,j in neighbours:
            queue.append((i,j))
        
        a_edges=False
        for i,j in neighbours:
            a_edges = edges[i,j] or a_edges #We require one of the neigbours to be an edge.
        
        if(weak_edges[x,y] and a_edges):
            edges[x,y]=True
