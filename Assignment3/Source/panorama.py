...
def harris_corners(img, window_size=3, k=0.04):
    ...
    ### YOUR CODE HERE
    dx2=convolve(dx**2,window,mode="constant")
    dy2=convolve(dy**2,window,mode="constant")
    dxy=convolve(dx*dy,window,mode="constant")
    detM=dx2 * dy2 -(dxy**2)
    traceM=dx2+dy2
...
def simple_descriptor(patch):
    ...
    if sd ==0:
        sd=1
    feature= ((patch-mean)/sd).reshape(-1)
...
#     N = desc1.shape[0]
    ...
    for index,d in enumerate(dists):
        shortest_index=np.argmin(d)
        sort_vector=np.sort(d)
        if (sort_vector[0]/sort_vector[1]) < threshold:
            matches.append([index,shortest_index])
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for x in range(n_iters):
        randmatchindex=np.random.choice(N,n_samples,replace=False)
        rmatched1=matched1[randmatchindex]
        rmatched2=matched2[randmatchindex]
        H=np.linalg.lstsq(rmatched2,rmatched1,rcond=None)[0]
        H[:,2] = np.array([0, 0, 1])
        inliers=np.sqrt(((matched2.dot(H) - matched1)**2).sum(axis=-1)) < threshold
        ncurrent_inliers=inliers.sum()
        if ncurrent_inliers>n_inliers:
            max_inliers=inliers
            n_inliers=ncurrent_inliers
    ### END YOUR CODE
    H=np.linalg.lstsq(matched2[max_inliers],matched1[max_inliers],rcond=False)[0]
    H[:,2] = np.array([0, 0, 1])
    print(H)
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for x in range(rows):
        for y in range(cols):
            cell_G = G_cells[x,y]
            cell_theta = theta_cells[x,y]
            for a in range(G_cells[x,y].shape[0]):
                for b in range(G_cells[x,y].shape[1]):
                    binpos=(cell_theta[x,y]//n_bins) % n_bins
                    cells[x,y,int(binpos)]+=cell_G[a,b]
            cells[x,y,:] = cells[x,y,:] / np.sum(cells[x,y,:])
